...
void TSerialize::readString(TStream& ifs,
                      std::string& dst) {
  int sz = ifs.readu16le();
  dst = "";
  for (int i = 0; i < sz; i++) dst += ifs.get();
}

void TSerialize::writeString(TStream& ofs,
                        const std::string& src) {
  ofs.writeu16le(src.size());
  for (unsigned int i = 0; i < src.size(); i++) ofs.put(src[i]);
}
