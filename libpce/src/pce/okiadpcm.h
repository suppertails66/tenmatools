/**
 * OKI-ADPCM decoder lifted from Mednafen (and modified to be self-contained).
 */
...
extern const int32_t OKIADPCM_DeltaTable[49][16];
...
class OKIADPCM_Decoder
{
 ...
 inline uint16_t GetSample(void)
 {
  return(CurSample);
 }

 inline void SetSample(uint16_t new_sample)
 {
  assert(new_sample <= 0xFFF);
  CurSample = new_sample;
 }

 inline uint8_t GetSSI(void)
 {
  return(StepSizeIndex);
 }

 inline void SetSSI(uint8_t new_ssi)
 {
  assert(new_ssi <= 48);
 ...
 inline int32_t DecodeDelta(const uint8_t nibble)
 {
  int32_t ret = OKIADPCM_DeltaTable[StepSizeIndex][nibble];
 ...
 inline uint16_t Decode(const uint8_t nibble)
 {
...
class OKIADPCM_Encoder
{
 ...
 uint8_t EncodeSample(uint16_t in_sample)
 {
  uint8_t nibble = 0;
  int32_t sample_delta = in_sample - Accum;
  int piece;
  ...
  nibble = ((uint32_t)(sample_delta >> 31) & 0x8) | piece;
